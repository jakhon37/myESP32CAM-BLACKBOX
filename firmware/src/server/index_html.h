...
<script>
  ...
  function btnFeedback(id, type) {
    ...
    if (type === 'loading') {
      ...
      return () => { btn.disabled = false; btn.className = oldClass; };
    }

    if (type === 'success') {
      btn.className = 'success';
      btn.textContent = 'OK!';
      setTimeout(() => { btn.className = oldClass; btn.textContent = oldTxt; }, 1000);
    }
  }
  ...
  function takeSnapshot() {
    ...
    img.onerror = () => { done(); alert('Capture failed'); };
  }
  ...
  function toggleLed(type) {
    ...
    fetch(CAM + '/led?type=' + type + '&state=' + newState)
      ...
      .catch(() => { done(); alert(type + ' LED failed'); });
  }
  ...
  function toggleRecording() {
    ...
    const policy = document.getElementById('rec-policy').value;
    ...
    fetch(CAM + '/record?action=' + action + '&policy=' + policy)
      ...
      .catch(() => { done(); alert('Recording command failed'); });
  }
  ...
  function updateRecordingUI(d) {
    ...
    if (recording) {
      ...
      btn.className = 'active';
      dot.className = 'blink';
      status.textContent = 'REC: ' + d.duration + 's';
  ...
  function refreshStatus() {
    ...
    // check recording status too
    fetch(CAM + '/record')
      .then(r => r.json())
      .then(d => updateRecordingUI(d));
  }
  ...
  if (savedSize) {
    document.getElementById('stream').style.maxHeight = savedSize;
    document.getElementById('size-select').value = savedSize;
  }
  
  stopStream(); // ensures UI is in IDLE state
  refreshStatus();
  refreshSD();
  setInterval(refreshStatus, 10000);

  // faster updates for recording duration
  setInterval(() => {
    if (recording) {
      fetch(CAM + '/record')
        .then(r => r.json())
